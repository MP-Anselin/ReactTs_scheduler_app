...
const TaskForm: React.FC = () => {
  const { date, task, addTask, saveTask, setDate, deleteTask } =
  ...
  const _saveTask = () => {
    const checkError = [name.trim().length || 0];

    const hasError = actions.find((el) => {
      const values = Object.values(el);
      for (const index in values) if (!values[index]) return true;
      return false;
    });
      ...
      <div className="task-form">
        ...
        <div className="center">
          <div className="row1">
            <label>Actions</label>
            &nbsp;&nbsp;
            <button className="button button-green" onClick={addAction}>
              +
            </button>
          </div>
          <br />
          <div className="overFlowYAction">{returnHtmlActionList()}</div>
        </div>
        ...
        {error ? (
          <p className="error">All the fields of the task is required</p>
        ) : null}
      </div>
    </Modal>
  );
